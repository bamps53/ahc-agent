...
import yaml # for verifying yaml.dump calls
...
class TestInitService:
    ...
    def test_initialize_project_success_with_args_and_workspace(self, mock_file_open, mock_path_mkdir, mock_yaml_dump, mock_scraper, mock_config, tmp_path):
        ...
        result = init_service.initialize_project(
            ...
            template=custom_template_arg,
            docker_image=custom_image_arg,
        ...
        dumped_file_stream = mock_yaml_dump.call_args[0][1]
    ...
    def test_initialize_project_success_defaults_no_workspace(self, mock_file_open, mock_path_mkdir, mock_yaml_dump, mock_scraper, mock_config, tmp_path):
    ...
    def test_initialize_project_mkdir_fails(self, mock_path_mkdir, mock_config, tmp_path):
        ...
        assert "Mocked FileExistsError" in str(exc_info.value)
    ...
    def test_initialize_project_scraper_fails(self, mock_scraper, mock_yaml_dump, mock_file_open, mock_path_mkdir, mock_config, tmp_path):
        ...
        # To check logger calls, we patch 'logging.getLogger.error'
        with patch.object(init_service.logger, 'error') as mock_logger_error:
    ...
    def test_initialize_project_uses_config_defaults_if_args_not_provider(self, mock_config, tmp_path):
        ...
        with patch("ahc_agent.services.init_service.scrape_and_setup_problem") as mock_scraper, \
             patch("ahc_agent.services.init_service.yaml.dump") as mock_yaml_dump, \
    ...
    def test_initialize_project_overrides_config_with_args(self, mock_config, tmp_path):
        ...
        with patch("ahc_agent.services.init_service.scrape_and_setup_problem"), \
             patch("ahc_agent.services.init_service.yaml.dump") as mock_yaml_dump, \
             patch("pathlib.Path.mkdir"), \
             patch("builtins.open", new_callable=mock_open):
