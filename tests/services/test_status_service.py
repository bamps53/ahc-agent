...
class TestStatusService:
    ...
    def test_get_status_watch_mode_stops_if_session_vanishes(self, mock_format_ts, mock_sleep, mock_config, mock_kb):
        ...
        with patch.object(service, "_format_session_status", return_value=["Initial status"]) as spy_format_status, patch(
            "ahc_agent.services.status_service.logger.info"
        ) as mock_logger_info:  # Spy on logger
            result = service.get_status(session_id="s1", watch=True)
            ...
            assert any("Session s1 no longer found. Stopping watch." in log_msg for log_msg in log_calls)
    ...
    def test_get_status_watch_mode_keyboard_interrupt(self, mock_sleep, mock_config, mock_kb):
        ...
        with patch.object(service, "_format_session_status", return_value=["Status during watch"]) as spy_format, patch(
            "ahc_agent.services.status_service.logger.info"
        ) as mock_logger_info:
            result = service.get_status(session_id="s1", watch=True)
