...
class TestBatchService:
    ...
    async def test_evaluate_solution_for_experiment(self, mock_llm_client_bs, mock_docker_manager_bs, mock_config_bs):
        ...
        # AsyncMockのreturn_valueを使用して、各呼び出しの戻り値を設定
        mock_debugger_instance.compile_and_test.side_effect = [
            {"success": True, "execution_output": "out1", "execution_time": 0.1},
            {"success": False, "compilation_errors": "err", "execution_errors": None},
        ]
        mock_score_calc.side_effect = [100.0]  # Only for the successful one

        avg_score, details = await service._evaluate_solution_for_experiment(test_code, test_cases, mock_score_calc, mock_debugger_instance)
    ...
    async def test_run_single_experiment_service_success(
        ...
        mock_os_makedirs,
        mock_path_is_file,
        mock_file_open_bs,  # Renamed mock_file_open
        MockKnowledgeBase,
        MockProblemAnalyzer,
        ...
        mock_pa_instance.analyze = AsyncMock(return_value={"analysis_key": "analysis_val"})
        mock_ss_instance = MockSolutionStrategist.return_value
        mock_ss_instance.develop_strategy = AsyncMock(return_value={"strategy_key": "strategy_val"})
        ...
        mock_ee_instance.evolve = AsyncMock(
            return_value={
                "best_solution": "best_code_single_exp",
                "best_score": 120.0,
                "generations_completed": 45,
                "evolution_log": {"log_data": []},
            }
        )
        ...
        with patch("json.dump") as _:
            ...
            assert any(
                call(result_json_path, "w", encoding="utf-8") in mock_file_open_bs.call_args_list
                for call_obj in mock_file_open_bs.mock_calls
                if call_obj[0] == ""
            )
    ...
    async def test_run_batch_experiments_service_valid_config(
        self,
        mock_run_single_exp,
        mock_os_makedirs_batch,
        mock_file_open_batch,
        mock_yaml_safe_load,
        mock_llm_client_bs,
        mock_docker_manager_bs,
        mock_config_bs,
        ...
        # async generatorの代わりにリストを使用
        mock_run_single_exp.side_effect = [
            {"experiment_id": "P1_PS1_repeat1", "best_score": 100, "error": None},
            {"experiment_id": "P1_PS1_repeat2", "best_score": 110, "error": None},
        ]
